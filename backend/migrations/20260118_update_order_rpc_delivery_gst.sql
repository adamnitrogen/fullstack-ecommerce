-- Migration: Update create_order_transactional RPC to handle delivery_gst
-- File: 20260118_update_order_rpc_delivery_gst.sql
-- Run after: 20260117_add_delivery_gst_to_orders.sql

-- Update the RPC function to handle new delivery fields
...
BEGIN
    ...
    -- CREATE ORDER (with delivery_gst)
    INSERT INTO orders (
        user_id,
        order_number,
        customer_name,
    ...
    SELECT
        ...
        COALESCE((p_order_data->>'delivery_gst')::NUMERIC, 0),  -- Handle delivery_gst
    ...
    -- CREATE ORDER ITEMS (with delivery_gst and snapshot)
    ...
    LOOP
        ...
        INSERT INTO order_items (
            ...
            delivery_charge,
            delivery_gst,  -- NEW COLUMN
            delivery_calculation_snapshot,  -- NEW COLUMN
            coupon_id,
            coupon_code,
            coupon_discount
        ) VALUES (
            ...
            COALESCE((v_item->>'delivery_gst')::NUMERIC, 0),  -- Handle delivery_gst
            v_item->'delivery_calculation_snapshot',  -- Handle snapshot
...
COMMENT ON COLUMN orders.delivery_gst IS 'GST amount on delivery charges';
COMMENT ON COLUMN order_items.delivery_gst IS 'GST amount on delivery charge for this item';
COMMENT ON COLUMN order_items.delivery_calculation_snapshot IS 'Snapshot of delivery charge calculation for audit trail (JSONB)';

-- Verify the function was updated
SELECT routine_name, routine_definition 
FROM information_schema.routines 
WHERE routine_name = 'create_order_transactional';
