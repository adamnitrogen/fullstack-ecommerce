-- Migration: Fix Order Transaction Function (Use 'inventory' instead of 'stock')
-- Purpose: Update function to use the correct column name for stock checks
...
BEGIN
    ...
    -- 5. DECREASE INVENTORY (Fixed column name from 'stock' to 'inventory')
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_order_items)
    LOOP
        v_product_id := (v_item->>'product_id')::UUID;
        v_quantity := (v_item->>'quantity')::INT;
        
        -- Check stock availability (using 'inventory' column)
