...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'order_items' AND column_name = 'delivery_charge') THEN
        ALTER TABLE order_items ADD COLUMN delivery_charge NUMERIC DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'order_items' AND column_name = 'delivery_gst') THEN
        ALTER TABLE order_items ADD COLUMN delivery_gst NUMERIC DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'order_items' AND column_name = 'total_amount') THEN
        ALTER TABLE order_items ADD COLUMN total_amount NUMERIC DEFAULT 0;
    END IF;

    -- Snapshot columns (JSONB)
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'order_items' AND column_name = 'delivery_calculation_snapshot') THEN
        ALTER TABLE order_items ADD COLUMN delivery_calculation_snapshot JSONB;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'order_items' AND column_name = 'coupon_code') THEN
        ALTER TABLE order_items ADD COLUMN coupon_code TEXT;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'order_items' AND column_name = 'coupon_discount') THEN
        ALTER TABLE order_items ADD COLUMN coupon_discount NUMERIC DEFAULT 0;
    END IF;
END $$;

-- 2. Update RPC to populate these columns
...
BEGIN
    ...
    -- 2. CREATE ORDER ITEMS (Updated with Tax & Delivery Columns)
