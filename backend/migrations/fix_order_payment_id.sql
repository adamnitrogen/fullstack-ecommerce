-- Migration: Fix Order Transaction Function (Add payment_id to order)
-- Purpose: Include payment_id when creating the order record
...
BEGIN
    ...
    -- 1. CREATE ORDER (Now includes payment_id!)
    INSERT INTO orders (
        user_id,
        order_number,
        payment_id,  -- ADDED: Link to payments table
        "customerName",
        "customerEmail",
        "customerPhone",
        shipping_address_id,
    ...
    SELECT
        p_user_id,
        v_order_number,
        p_payment_id,  -- ADDED: Include payment_id value
    ...
    -- 3. LINK PAYMENT TO ORDER (Bidirectional - also update payments.order_id)
